...
class Utilities {
  ...
  static Future<void> createTestRun() async {
    newRun = await TestRun.create(
        name: '${DateTime.now().toIso8601String()} Test Run To Do',
        projectId: 3);
  ...
  static Future<void> tap(Finder element, WidgetTester tester,
...
/*  static Future<String> getText(Finder element,{required int testCaseId}) async {
  ...
  static Future<void> waitForElement(Finder element, WidgetTester tester,
      {required int testCaseId}) async {
    try {
  ...
  static Future<void> fillTextThenWait(
      Finder element, String text, WidgetTester tester,
      {required int testCaseId}) async {
    try {
  ...
  static Future<void> waitForElementAbsent(Finder element, WidgetTester tester,
      {required int testCaseId}) async {
    try {
      await tester.pump(Duration(seconds: 2));
  ...
  static Future<void> isPresented(Finder element, WidgetTester tester,
      {required int testCaseId}) async {
    try {
      expect(await Utilities.isPresent(element, tester, testCaseId: testCaseId),
          true);
      await setTestStatus(testCaseId, passed);
    } on TimeoutException catch (e) {
      await setTestStatus(testCaseId, failed);
